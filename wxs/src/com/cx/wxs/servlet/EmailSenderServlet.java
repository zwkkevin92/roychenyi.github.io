 ...
 * 邮件发送服务
 * @author 陈义
 * @date   2015-11-25 下午4:47:37
 */
public class EmailSenderServlet extends GenericServlet implements Runnable{
			...
			try{
				...
				if(mails.size()>0){
					for(;i<mails.size();i++){
						...
						System.out.println( mail.getContent().toString());
						mail.getSubject();
						
						mail.setFrom(new InternetAddress(sender,"DLOG"));
						if(userDNSQuery){
							//特快专递
							...
							for(int ai=0;ai<answers.length;ai++){
						        ...
						        System.out.println("Using " + answers[i].getAdditionalName()+" to send mail to " + email);
